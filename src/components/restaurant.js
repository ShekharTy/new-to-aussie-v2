...
function RestaurantSearch() {
  ...
  const parseCSV = (file) => {
    Papa.parse(file, {
      ...
      complete: (result) => {
        ...
        const allRestaurants = result.data.map(row => ({
        ...
        const uniqueCuisines = [...new Set(allRestaurants.map(restaurant => restaurant.cuisine))];
        setAllCuisines(uniqueCuisines);
        setFilteredCuisines(uniqueCuisines); 
        setRestaurants(allRestaurants);
  
        // Extract unique suburb names
        const suburbs = result.data.map(row => row['Suburb']).filter(suburb => suburb);
  ...
  const handleCuisineChange = (event) => {
    ...
    if (allCuisines && allCuisines.length > 0) {
      if (value.length > 0) {
        const filtered = allCuisines.filter(cuisine => cuisine.trim().toLowerCase().startsWith(value));
  ...
  const handleSuburbChange = (event) => {
    ...
    if (value) {
      ...
      const filtered = allSuburbs.filter(suburb => suburb.trim().toLowerCase().startsWith(trimmedValue));
  ...
  return (
    <div>
      ...
      <div className="relative bg-cover bg-center h-64 flex flex-col justify-center items-center text-white">
        <img src={bannerImage} alt="Banner" className="absolute inset-0 h-full w-full object-cover opacity-100" />
        <div className="absolute top-1/3 transform -translate-y-1/2 text-center">
          ...
          <p className="text-xxl text-stone-50 text-decoration-thickness: 2px;">Discover amazing dining experiences</p>
        </div>
      </div>
      ...
      <div className="body flex flex-col items-center mt-8 ml-64">
        <div className="bg-gray-200 p-4 w-3/4 rounded-lg shadow-lg">
          <ul className="mt-4 grid grid-cols-2 gap-4">
            <li
              className={`cursor-pointer bg-white rounded-md shadow-lg p-4 ${selectedModule === 'Search Restaurant' ? 'border-2 border-blue-600' : ''}`}
              onClick={() => setSelectedModule('Search Restaurant')}
            >
              <h3 className="text-xl font-semibold">Search Restaurant by Cuisine</h3>
              <p className="text-gray-600">Find restaurants based on your preferred cuisine.</p>
            </li>
            <li
        className={`cursor-pointer bg-white rounded-md shadow-lg p-4 ${selectedModule === 'Play Spin Wheel' ? 'border-2 border-blue-600' : ''}`}
        onClick={() => setSelectedModule('Play Spin Wheel')}
           >
        <h3 className="text-xl font-semibold">Play Spin Wheel</h3>
        <p className="text-gray-600">Content for Play Spin Wheel goes here.</p>
        </li>
          </ul>
        </div>
      </div>
      ...
      <div className="search-bar flex flex-col mt-8 items-center">
        <div className="mb-4">
          <label htmlFor="cuisine" className="mr-4 text-lg text-blue-800 font-bold">Type your preferred cuisine here:</label>
          <input
            type="text"
            id="cuisine"
            className="border-2 border-gray-300 bg-cyan-600 h-10 px-5 pr-10 rounded-lg text-lg text-stone-50 focus:outline-none w-96"
        ...
        <div>
          <label htmlFor="suburb" className="mr-4 text-lg  text-blue-800 font-bold">Type your preferred suburb here:</label>
          <input
            type="text"
            id="suburb"
            className="border-2 border-gray-300 bg-cyan-600 h-10 px-5 pr-10 rounded-lg text-lg text-stone-50 focus:outline-none w-96"
      ...
      {/* Display filtered restaurants here */}
      <div className="filtered-restaurants mt-6">
        <h2 className="text-xl font-bold mb-2"> List of restaurants with preferred cuisine and suburb below!</h2>
        <div className="grid grid-cols-4 gap-4 animate__animated animate__fadeInUp" style={{ backgroundColor: '#EFFEFC' }}>
          {filteredRestaurants.map((restaurant, index) => (
            <div key={index} className="border p-4">
              <h3 className="text-lg font-semibold">{restaurant.name}</h3>
              <p className="text-gray-600">Suburb: {restaurant.suburb}</p>
              <a href={restaurant.googleLink} target="_blank" rel="noopener noreferrer" className="text-blue-600 underline hover:text-blue-800">Google Link</a>
            </div>
          ))}
        </div>
      </div>
      ...
      {filteredRestaurants.length === 0 && selectedCuisine && selectedSuburb && (
        <div className="filtered-restaurants mt-6">
          <h2 className="text-xl font-bold mb-2">No restaurants found with the selected cuisine in the selected suburb!</h2>
        </div>
      )}
      {filteredRestaurants.length === 0 && selectedCuisine && !selectedSuburb && (
        <div className="filtered-restaurants mt-6">
          <h2 className="text-xl font-bold mb-2">No restaurants found with the selected cuisine!</h2>
        </div>
      )}
